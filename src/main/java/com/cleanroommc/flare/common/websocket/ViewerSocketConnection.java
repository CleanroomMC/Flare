...
import com.cleanroommc.flare.proto.FlareWebSocketProtos.RawPacket;
...
public class ViewerSocketConnection implements BytesocksClient.Listener, AutoCloseable {
    ...
    public void onText(CharSequence data) {
        try {
            ...
            this.flare.logger().warn("Exception occurred while reading data from the socket", e);
        }
    }

    @Override
    public void onError(Throwable error) {
        this.flare.logger().error("Socket error: {} {}", error.getClass().getName(), error.getMessage(), error);
    }

    @Override
    public void onClose(int statusCode, String reason) {
        this.flare.logger().warn("Socket closed with status: {}, with reason: {}", statusCode, reason);
    }
    ...
    public void sendPacket(Consumer<PacketWrapper.Builder> packetBuilder) {
        ...
        try {
            ...
            this.flare.logger().error("Exception occurred while sending data to the socket.", e);
        }
    }
