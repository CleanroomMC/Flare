...
public interface ThreadDumper {
     ...
     */
    ThreadInfo[] dump(ThreadMXBean threadBean);
     ...
     */
    boolean threadIncluded(long threadId, String threadName);
    ...
    ThreadDumper ALL = new ThreadDumper() {
        @Override
        public ThreadInfo[] dump(final ThreadMXBean threadBean) {
    ...
    final class Specific implements ThreadDumper {
        ...
        public Set<String> threadNames() {
            if (this.threadNamesLowerCase == null) {
                this.threadNamesLowerCase = threads().stream()
        ...
        public boolean threadIncluded(long threadId, String threadName) {
            ...
            return threadNames().contains(threadName.toLowerCase());
        }
    ...
    final class Regex implements ThreadDumper {
        ...
        public ThreadInfo[] dump(ThreadMXBean threadBean) {
            return this.threadFinder.getThreads()
                    .filter(thread -> threadIncluded(thread.getId(), thread.getName()))
